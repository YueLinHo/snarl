...
'    snp3_GotPacketEnd = (InStr(Packet, vbCrLf & "END" & vbCrLf & vbCrLf) > 0)
    snp3_GotPacketEnd = (g_SafeRightStr(Packet, 7) = vbCrLf & "END" & vbCrLf)
...
Dim sz As String
    ...
    g_Debug "snp3_IsSNP3()", LEMON_LEVEL_PROC_ENTER

    sz = uGetFirstLine(Packet, vbCrLf)
    g_Debug "first line is " & g_Quote(sz)
...
Static i As Long
    ...
    Debug.Print "** snp3_Translate(): RawPacket follows..."
...
Dim sz As String
    ...
    ' /* get the header - entries ("words") are split by space characters */

    ppd.EntryAt 1, sz, ""

    ' /* if the first word isn't "SNP/3.0" then we fail with SNARL_ERROR_UNSUPPORTED_VERSION */
    ...
    ElseIf uIsForwardHeader(sz) Then
        Debug.Print "snp3_Translate(): is forward packet"
        Flags = Flags Or SN_NF_FORWARD
...
Private Function uIsResponseHeader(ByVal Header As String) As Boolean
    ...
    ' /* very simplistic check for the moment - simply looks for the occurance of one of
    '    "OK", "FAILED" or "CALLBACK" with a precededing space */

    uIsResponseHeader = (InStr(Header, " OK") > 0) Or (InStr(Header, " FAILED") > 0) Or (InStr(Header, " CALLBACK") > 0)

End Function

Private Function uIsForwardHeader(ByVal Header As String) As Boolean
...
Public Function snp_CreateForward(ByRef Info As T_NOTIFICATION_INFO) As String
    ...
    snp_CreateForward = "SNP/3.0 FORWARD" & vbCrLf & _
